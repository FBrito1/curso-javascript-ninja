(function() {
  ...
  */
  ...
  const fullName = 'fabio-brito'
  const arrayName = fullName.split('-').map((name) => {
  ...
  */
  ...
  const phrase = names.reduce((accumulator, item, index) => {
  ...
  */
  ...
  for(let i = 0; i < myName.length; i++) {
    if(i % 2 === 0) {
      nameArray.push(myName.charAt(i).toUpperCase())
    } else {
      nameArray.push(myName.charAt(i).toLowerCase())
    }
  }
