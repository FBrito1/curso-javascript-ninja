(function(win, doc) {
  ...
  */
  $button.addEventListener('click', function (event) {
    ...
    let confirm = win.confirm('Tem certeza que deseja enviar o formulário?');
    confirm ? console.log('Enviado com sucesso!') : console.log('Não enviado.');
  ...
  */
  ...
  function isValidEmail (email) {
    let validEmailRegex = /[\w_.+]+@\w+.\w{2,}(?:.\w{2,})/g;
    let match = email.match(validEmailRegex);

    if(match) {
      return match.includes(email) ? true : false;
    }

    return false;
  }

  console.log(isValidEmail('fabio'));
